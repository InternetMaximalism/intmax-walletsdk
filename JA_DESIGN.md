...
以下は、Webmax プロトコルの利用例です。ただし、Webmax プロトコルはただの通信規格に過ぎず、様々な応用が可能です。

### Connect with Web Wallet

最もオーソドックスな使い方として、Dapp に Webmax の DappSDK を組み込むことで、Webmax に対応した Web ウォレットと接続できます。
これは、WalletConnect のようなプロトコルと同じ利用方法です。

### Bookmarklet Wallet

ブックマークを利用して、webmax の DappSDK を、Dapp ページに挿入することで、ページの window.ethereum から、Webmax 対応の Web ウォレットを利用できるようにします。
これにより、Metamask などのブラウザ拡張機能型のウォレットと同じような UX で Web ウォレットを利用できます。
しかし、CSP などのセキュリティポリシーにより、一部の Dapp では利用できない場合や、ブックマークレットそのものの UX が悪いという問題があります。

### Webmax Wallet Luncher

前述のとおり、ブックマークレットウォレットは、UX やセキュリティの問題があります。
しかし、Webmax 対応の Web ウォレットを管理するようなランチャーを、ブラウザ拡張機能として提供すれば、この問題を解決できます。
この拡張機能があれば、Web ウォレットを、拡張機能型のウォレットと全く同じような UX で利用でき、セキュリティも拡張機能型のウォレットと同じように保証されます。
このユースケースは、Web ウォレットにとっては革新的で、現在開発中です。
